...
# Przygotuj nowy wpis w jednej linii
NEW_ENTRY=$(echo -n "{\"version\":\"2.2\",\"timestamp\":$TIMESTAMP,\"date\":\"$DATE_READABLE\",\"host\":\"$HOSTNAME\",\"device\":\"/dev/nvme0n1\",\"metrics\":{\"total_written_gb\":$WRITTEN_GB,\"total_read_gb\":$READ_GB,\"temperature\":$TEMPERATURE},\"message\":\"NVMe stats - Written: ${WRITTEN_GB}GB, Read: ${READ_GB}GB, Temp: ${TEMPERATURE}\"}")

# Zapisz nowy wpis jako nowa linie w pliku
echo "$NEW_ENTRY" >> "$JSON_LOG_FILE"
